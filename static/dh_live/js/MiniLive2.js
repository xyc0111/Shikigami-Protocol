...
let CONFIG = {
    ...
    // baseDir：所有相对资源的前缀。原版留空（页面与资源同目录）；
    // 被嵌入到别的页面时由宿主设置为绝对前缀，如 "/dh_live/"。
    baseDir: "",
    ...
    textureBsSrc: "common/bs_texture_halfFace.png",  // 表情基纹理路径
    // 绿幕抠图参数配置
    chromaKey: {
...
/** 把相对资源路径按 CONFIG.baseDir 展开；绝对路径（/ http: data: blob:）原样返回。 */
function dhResolveAsset(p) {
    if (!p) return p;
    if (/^([a-z]+:)?\/\//i.test(p) || p.startsWith('/') || p.startsWith('data:') || p.startsWith('blob:')) return p;
...
function setupBackgroundVideo() {
    ...
    if (bgVideo) {
        bgVideo.src = dhResolveAsset(CONFIG.backgroundVideoSrc);
        bgVideo.load();
...
/**
 * 切换数字人形象（供宿主 / 桥接层调用）。
 * @param {string} dir 相对于 CONFIG.baseDir 的形象目录名，如 "avatars/dh_male_01"
 */
async function dhSwitchAvatar(dir) {
    isPaused = true;
    const msg = document.getElementById('startMessage');
    if (msg) msg.style.display = 'block';
    asset_dir = dir;
    await videoProcessor.init(dhResolveAsset(dir + "/01.mp4"), dhResolveAsset(dir + "/combined_data.json.gz"));
    await loadCombinedData();
    await setupVertsBuffers();
...
window.dhSwitchAvatar = dhSwitchAvatar;
// 初始化处理器
...
//            gl.bindTexture(gl.TEXTURE_2D, null);
        ...
        image.src = dhResolveAsset(CONFIG.textureBsSrc || 'common/bs_texture_halfFace.png');
}
...
async function newVideoTask() {
    await videoProcessor.init(dhResolveAsset(CONFIG.videoSrc), dhResolveAsset(CONFIG.dataSrc));
    await loadCombinedData();
    await init_gl();
    await setupVertsBuffers();
    initMemory();
